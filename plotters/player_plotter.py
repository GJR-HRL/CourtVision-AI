...
class PlayerTracksPlotter:
    ...
    def plot(self,video_frames,tracks,player_assigment, ball_aquisition):
        """
        Draw player tracks indicators on a list of video frames.
        Args:
        ...
        for frame_num, frame in enumerate(video_frames):
            ...
            # Copy by value
            frame = frame.copy()
            players_dict = tracks[frame_num]

            # player_assigment is a list of dictionaries with  [frame_num][track_id] = team_id 
            #          [frame_number] [player_id] = team_id
            # player_assignment[0][10] = 1 or 2
            player_assignment_for_frame = player_assigment[frame_num]

            player_id_has_ball = ball_aquisition[frame_num]

            # Plot Ellipse around each player
            for track_id,player in  players_dict.items():
                ...
                frame = draw_ellipse(frame,player["bbox"],color,tracks_id = track_id)
                
                # If player has the ball then draaw triangle at head
