...
class TopViewConverter:
    def __init__(self, court_image_path):
        ...
        # transfering real court keypoints into mini map 
        # Eg.
        # height of map = 161 px , real court = 15 
        # first point is at 0,0 in mini map 
        # second point is 0.9 down in real court height to convert that in mini map we follow this formula 
        #   real-court ratio        mini-map ratio
        #    0.9                        x
        #    ---                =      ---  
        #    15 (court height)        161 px 

        #   0.06  * 161 px      =    x 

        #   x = 9.66 px in mini-map

        # like-wise we find every other point
    ...
    def validate_keypoints(self, keypoints_list):
        """
        Validates detected keypoints by comparing their proportional distances to the top view keypoints.
        ...
        """
        # In summary:
        # Shallow copy is suitable when you need a new top-level object but can share references to nested mutable objects. It's faster and uses less memory.
        # Deep copy is necessary when you need a completely independent copy of an object, including all its nested mutable objects, to prevent unintended side effects. It's slower and uses more memory.
        ...
        for frame_idx, frame_keypoints in enumerate(keypoints_list):
            ...
            for i in detected_indices:
                # Skip if this is (0, 0) , doing it again because in loop we might overwrite it 
                ...
                # Calculate and compare proportions with 80% error margin
    ...
    def transform_players_to_top_view(self, keypoints_list, player_tracks):
        """
