...
class BallTracker:
    ...
    def get_object_tracks(self,frames , read_from_stub=False,stub_path=None):
       ...
       """
        ...
        for frame_num, detection in enumerate(detections):
            ...
            tracks.append({})
            # if more then one ball is detacted then the one with max confidence is selected
            chosen_bbox = None 
            max_confidence = 0
    ...
    def remove_wrone_detections(self,ball_positions):
        """
        ...
        for i in range(len(ball_positions)):
            # checks if we have the ball_positions for every frames if not we return empty dictionary or list
            ...
            if last_good_frame_index == -1:
                # First valid detection so we continue as we cant compare to the previous one
            ...
            # current_box[:2] = top-left corner of the detected box.
            ...
            # Someone watching you from above sees:
            # → You didn’t move 3
            # → You didn’t move 4

            # You moved 5 steps diagonally.

            # Your actual displacement is 5.
            # Same logic for bounding boxes.


            if np.linalg.norm(np.array(last_good_box[:2] - np.array(current_box[:2]))):
                ball_positions[i] = {}
    ...
    def interpolate_ball_position(self,ball_positions):
        ...
        # [1,2,3, nan ,5] ->  after [1,2,3,4,5]
        # Interpolate() fills any NaN that has a value before and after it.

        # Use KALMAN filter for better performace
        # Position t+1​ = Position t ​+ Velocity t​
        df_ball_positions =  df_ball_positions.interpolate(method="linear",limit_direction="both")

        # [[100, 120]] -> [{1: {"bbox": [100, 120]}}] , 1 is class_id
